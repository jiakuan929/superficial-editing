        ...
        **kwargs,
        ...
        if config is not None and hasattr(config, "tokenizer_name"):
            ...
            if "qwen" in config.model_name.lower():
                print('Qwen tokenizer detected, Set pad token id and right padding')
                tokenizer.eos_token = "<|endoftext|>"
                tokenizer.pad_token = "<|endoftext|>"
                tokenizer.unk_token = "<|endoftext|>"
                # tokenizer.padding_side = 'left'
                # print('QwenTokenizer Detected, Set pad token id and left padding!!!')
            if "llama-3" in tok_name.lower():
                tokenizer.pad_token_id = tokenizer.eos_token_id
                tokenizer.padding_side = 'right'
                print("Llama-3 tokenizer detected, Set pad token id and right padding.")
            self.tok = tokenizer
