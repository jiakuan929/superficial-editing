...
def compute_zs(
    ...
    target_ids = tok.encode(
        request["target_new"]["str"], return_tensors="pt", add_special_tokens=False
    ).to(f"cuda")[0]
    ...
    if (
        "qwen" in model.config._name_or_path.lower()
    ...
    def edit_output_fn(cur_out, cur_layer):
        ...
        if cur_layer == hparams.mlp_module_tmp.format(layer):
            ...
            for i, idx in enumerate(lookup_idxs):
                cur_out[i, idx, :] += delta_mlp.to(cur_out.device)
        if cur_layer == hparams.attn_module_tmp.format(layer):
            ...
            # Add intervened delta
            for i, idx in enumerate(lookup_idxs):
                cur_out[i, idx, :] += delta_attn.to(cur_out.device)
        return cur_out
    ...
    for it in range(hparams.v_num_grad_steps):
        ...
        max_prob = (
            torch.exp(
                (max_probs * mask.to(max_probs.device)).sum(1) / target_ids.size(0)
            )
            .mean()
            .item()
        )
        ...
        weight_decay = hparams.v_weight_decay * (
            torch.norm(delta_mlp)
            / torch.norm(target_init_mlp.to(delta_mlp.device)) ** 2
            + torch.norm(delta_attn)
            / torch.norm(target_init_attn.to(delta_attn.device)) ** 2
        )
        ...
        if delta_mlp.norm() > max_norm.to(delta_mlp.device):
            with torch.no_grad():
                delta_mlp[...] = delta_mlp * max_norm.to(delta_mlp.device) / delta_mlp.norm()

        max_norm = hparams.clamp_norm_factor * target_init_attn.norm()
        if delta_attn.norm() > max_norm.to(delta_attn.device):
            with torch.no_grad():
                delta_attn[...] = delta_attn * max_norm.to(delta_attn.device) / delta_attn.norm()

    target_attn = target_init_attn + delta_attn.to(target_init_attn.device)    # For multi-gpus
    target_mlp = target_init_mlp + delta_mlp.to(target_init_mlp.device)
    print(
