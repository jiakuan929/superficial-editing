...
class BaseTrainer:
    def __init__(self, config, train_set: Dataset, val_set: Dataset):
        ...
        #     print(f"self.model.model.device = {self.model.model.device}")
        ...
        if 'minigpt4' in self.config.model_name.lower() or 'blip2' in self.config.model_name.lower():
            ...
            print("AutoMode Detected! using collate_gpt_fn")
            collate_fn = train_set.collate_gpt_fn
        ...
        self.output_dir = f"{config.results_dir}/{config.alg}_{config.model_code}_{config.data}_{run_date}"   # ./train_output/MEND
        os.makedirs(self.output_dir, exist_ok=True)
        # Save configurations     
        with open(self.output_dir + "/config.json", "w") as f:
            json.dump(OmegaConf.to_container(config), f, indent=4)

        model_dir = os.path.join(self.output_dir, "models")    # ./train_output/MEND/models
